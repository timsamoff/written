...
`app/wf.css` defines its own, separate token system rather than importing `style.css`'s. This is an intentional design relationship, not an oversight: Written (the publishing site) and Written & Formatted (the editor) are meant to share a consistent color and typography identity — the same visual "vibe" — while remaining two independently implemented stylesheets with no shared file. In practice this means the core palette tokens (`--color-bg`, `--color-accent`, and others) hold matching values, but not every same-named token does, and not every difference is a problem: `--shadow-sm`/`--shadow-md` are deliberately tinted differently in each file's own palette, and `--color-text`'s apparent mismatch is actually a correct match to a *different* role `wf.css` splits out (content text vs. UI chrome text) that `style.css` doesn't distinguish. Nothing in the code enforces any of this stays aligned going forward — keeping it that way is a manual, ongoing responsibility, not a structural guarantee. `CLAUDE.md` documents the specific token-by-token findings from the most recent reconciliation pass.
